...
//magica n mexe
char *adicionar(char *prezin, char add) {

	char *str = malloc (sizeof (char) * (strlen (prezin) + 2));
	
	int i = 0;
	while (prezin[i] != '\0') str[i] = prezin[i++];
...
void imprime(tp_no* raiz, char* l){
	if (raiz == NULL){
		return;
	}
	int i;
	...
	for (i = 0; i < LETRAS; i++){
		if(raiz->filhos[i] != NULL){
			...
			imprime(raiz->filhos[i], adicionar(l, i + 97));
			
		}
	}
}

void completa(tp_no* raiz, char* palavra){
	int i;
	int tam = strlen (palavra);
	for (i = 0; i < tam; i++){
		raiz = raiz->filhos[palavra[i] % 97];	
	} 
	
	 imprime(raiz, palavra);
}
